...
import KubaImg from '../images/contact/KubaImg.png'
import { motion } from 'framer-motion'
import { transition } from '../transitions'

const Contact = () => {
    return (
        <motion.section initial={{ opacity: 0, y: '100%' }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: '100%' }} transition={transition} className='section bg-white'>
            <div className='container mx-auto h-full'>
                <div className='flex flex-col lg:flex-row h-full items-center justify-start pt-36 gap-x-8 text-center lg:text-left'>
                    <motion.div initial={{ opacity: 0, y: '100%' }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: '100%' }} transition={transition} className='hidden lg:flex bg-[#EEF7F9] absolute bottom-0 left-0 right-0 top-72 -z-10'></motion.div>
                    ...
                    <motion.div initial={{ opacity: 0, y: '100%' }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: '100%' }} transition={{transition: transition, duration: 1.5}} className='lg:flex-1'>
                        <img src={KubaImg} alt='' />
                    </motion.div>
                </div>
            </div>
